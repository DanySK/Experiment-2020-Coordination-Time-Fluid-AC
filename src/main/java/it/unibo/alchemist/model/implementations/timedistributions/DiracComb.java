    ...
    protected final void updateStatus(
        ...
        if (param == 0) {
            if (!getNextOccurence().isInfinite()) {
                lastValidTime = getNextOccurence();
            }
            ...
            if (executed) {
                ...
                if (lastValidTime.compareTo(curTime) > 0) {
                    setTau(lastValidTime);
                } else {
                    var delta = (curTime.toDouble() - lastValidTime.toDouble()) % timeInterval;
                    var newTime = new DoubleTime(curTime.toDouble() - delta + timeInterval);
