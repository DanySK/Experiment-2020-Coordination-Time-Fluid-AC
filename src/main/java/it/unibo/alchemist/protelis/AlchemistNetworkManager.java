...
public final class AlchemistNetworkManager implements NetworkManager, Serializable {
    ...
    private LinkedHashMap<DeviceUID, MessageInfo> msgs = new LinkedHashMap<>();
    ...
    public Map<DeviceUID, Map<CodePath, Object>> getNeighborState() {
        ...
        final double currentTime = Optional.of(env.getSimulation())
            .map(Simulation::getTime)
            .map(Time::toDouble)
            .orElse(0.0);
        return convertMessages(m -> m.time + retentionTime > currentTime);
    }
